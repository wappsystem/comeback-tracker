                            ...
                            if( i<=lines.length && i<=NN && status=='ok') {
                                ...
                                if(items.length>=1){
                                    ...
                                    var query={UID:parseInt(uid)} 
                                        $vm.request({ cmd:'find',table:m.Table,query:query,skip:0,limit:1},function (res) {
                                            if (res.sys.permission == false) {
                                                $vm.alert("No permission. Private database table, ask the table's owner for permissions.");
                                                return;
                                            } 
                                            console.log(JSON.stringify(res.result[0]._id))       
                                            if (res.result.length > 0) {
                                                var p_rid=res.result[0]._id
                                                var data={};
                                                data=res.result[0].Data;
                                                data.Tsp_to_mg=tsp;
                                                var index={};
                                                index.I2 = res.result[0].I2;
                                                index.I3 = res.result[0].I3;
                                                index.I5 = res.result[0].I5;
                                                $vm.request({ cmd:'update',id:p_rid,table:m.Table,data:data,index:index}, function (res) {
                                                    if(res.status=="np"){
